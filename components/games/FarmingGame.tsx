...
import { ThemeMode, FarmingState, Plot, Crop, Decoration } from '../../types.ts';
...
interface FarmingGameProps {
  ...
  gameState: FarmingState;
  setGameState: (state: FarmingState) => void;
...
const FarmingGame: React.FC<FarmingGameProps> = ({ 
  onBack, currentTheme, gameState, setGameState, soundEnabled, onSave, onLoad
}) => {
  const theme = themes[currentTheme];
  const [selectedTool, setSelectedTool] = useState<'water' | 'harvest' | 'fertilizer' | string>('wheat');
  const [activeTab, setActiveTab] = useState<'farm' | 'shop'>('farm');
  const [floatText, setFloatText] = useState<{id: number, text: string, type: 'money' | 'xp' | 'level', x: number, y: number}[]>([]);
  ...
  const triggerFloatText = (text: string, type: 'money' | 'xp' | 'level', clientX: number, clientY: number) => {
      let x = 0; 
      let y = 0;
  ...
  const handlePlotClick = (plotId: number, e: React.MouseEvent) => {
  ...
  const buyDecoration = (id: string, e: React.MouseEvent) => {
  ...
  const getGrowthProgress = (plot: Plot) => {
  ...
  const handleToolClickCapture = (e: React.MouseEvent) => {
...
const ToolButton: React.FC<any> = ({ active, onClick, icon, label, theme, disabled }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className={`
