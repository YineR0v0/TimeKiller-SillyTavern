...
import { Grid2048, Direction, ThemeMode, Game2048State } from '../../types';
...
interface Game2048Props {
  ...
  onLoad: () => void;
  // State lifted up
  gameState: Game2048State | undefined;
  setGameState: (state: Game2048State) => void;
}
...
const Game2048: React.FC<Game2048Props> = ({ 
  ...
  const addRandomTile = (currentGrid: Grid2048) => {
    const available = [];
  ...
  // Initialize only if no state exists
  useEffect(() => {
    if (!gameState) {
  ...
  const move = (dir: Direction) => {
    ...
    // Rotate to standardize to "Left" movement logic
  ...
  const rotateLeft = (matrix: Grid2048) => {
  ...
  const checkGameOver = (currentGrid: Grid2048) => {
    for (let r = 0; r < SIZE; r++) {
      for (let c = 0; c < SIZE; c++) {
  ...
  const getCellColor = (val: number) => {
    if (val === 0) return 'bg-opacity-10 bg-black'; // Placeholder
