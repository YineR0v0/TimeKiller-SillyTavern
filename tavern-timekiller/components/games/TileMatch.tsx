...
import { ThemeMode, TileMatchState, MatchTile } from '../../types';
...
interface TileMatchProps {
  ...
  gameState: TileMatchState | undefined;
  setGameState: (state: TileMatchState) => void;
}
...
const TileMatch: React.FC<TileMatchProps> = ({ 
  ...
  const initGame = () => {
    let positions: {x: number, y: number, layer: number}[] = [];
    ...
    const newTiles: MatchTile[] = positions.map((pos, index) => ({
        id: index,
        type: typePool[index],
        gridX: pos.x,
        gridY: pos.y,
  ...
  const calculateClickability = (currentTiles: MatchTile[]) => {
